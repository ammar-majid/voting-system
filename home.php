...
<div class="containe-fluid">
	...
	$votingQuery = $conn->query("SELECT * FROM voting_list WHERE is_default = 1 ");
	$votingData = $votingQuery->fetch_assoc();
	
	if ($votingData) {
		$id = $votingData['id'];
		$votesQuery = $conn->query("SELECT * FROM votes WHERE voting_id = $id ");
		$v_arr = array();
	
		while ($row = $votesQuery->fetch_assoc()) {
		...
		$optsQuery = $conn->query("SELECT * FROM voting_opt WHERE voting_id = $id");
		$opt_arr = array();
	
		while ($row = $optsQuery->fetch_assoc()) {
	...
	<div class="row mt-3 ml-3 mr-3">
			...
			<div class="card">
				<div class="card-body">
					...
					<h3><b><?php echo isset($votingData['title']) ? $votingData['title'] : 'No Title Available' ?></b></h3>

					<small><b><?php echo isset($votingData['description']) ? $votingData['description'] : 'No Description Available' ?></b></small>
...
<?php
		...
		// Handle the case when no voting data is found
		echo '<h1 style="color: red; font-size: 50px; text-align: center;">No voting data found.</h1>';
